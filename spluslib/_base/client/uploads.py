...
class UploadMethods:
        ...
        Arguments
            ...
            file (`str` | `bytes` | `file` | `media`):
                ...
                * A Bot API-like ``file_id``. You can convert previously
                  ...
                  `spluspy.utils.pack_bot_file_id`.
            ...
            reply_to (`int` | `Message <spluspy.tl.custom.message.Message>`):
            ...
            parse_mode (`object`, optional):
                ...
                <spluspy.client.messageparse.MessageParseMethods.parse_mode>`
            ...
            buttons (`list`, `custom.Button <spluspy.tl.custom.button.Button>`, :tl:`KeyboardButton`):
            ...
            comment_to (`int` | `Message <spluspy.tl.custom.message.Message>`, optional):
                Similar to ``reply_to``, but replies in the linked group of a
                broadcast channel instead (effectively leaving a "comment to"
                the specified message).

                This parameter takes precedence over ``reply_to``. If there is
                no linked chat, `spluspy.errors.sgIdInvalidError` is raised.
            ...
            spoiler (`bool`, optional):
                If `True`, the photo/video is sent blurred behind a
                "tap to reveal" spoiler overlay, same as the official
                apps' spoiler toggle when attaching media. Only applies
                to photos and videos (quick-mode image/video uploads);
                ignored for other file types like documents or audio.

        Returns
            The `Message <spluspy.tl.custom.message.Message>` (or messages)
    ...
    async def upload_file(
        ...
        Returns
            ...
            `InputSizedFile <spluspy.tl.custom.inputsizedfile.InputSizedFile>`
        ...
        # SoroushPlus (unlike real Telegram) never accepts upload.SaveFilePart /
        # upload.SaveBigFilePart on the main/update connection -- confirmed by
        # capturing the official web client (GramJS fork): its uploadFile() /
        # downloadFile() *always* call client.getSender(dcId, ...) to obtain a
        # dedicated exported connection, even when dcId is the account's own
        # DC. It never sends media parts over the main sender at all. Doing so
        # here previously just hung/silently failed instead of raising the
        # RPCError we were watching for, since the main connection isn't wired
        # to accept these requests in the first place -- so start on a
        # borrowed exported sender for the current DC right away, the same
        # one the browser uses, instead of trying the main sender first.
        ...
        try:
            async with helpers._FileStream(file, file_size=file_size) as stream:
                ...
                for part_index in range(part_count):
                    ...
                    # Send this part. We're already on a dedicated borrowed
                    # sender (see above), but if the file turns out to live
                    # on a *different* DC than our own, the server tells us
                    # via FileMigrateError and we switch to a borrowed sender
                    # for that DC instead and keep going from there.
                    while True:
                        try:
                            result = await self._call(upload_sender, request)
                            break
                        except errors.FileMigrateError as e:
                            self._log[__name__].info('Upload belongs on another DC, switching sender')
                            await self._return_exported_sender(upload_sender)
                            borrowed_dc_id = e.new_dc
                            upload_sender = await self._borrow_exported_sender(borrowed_dc_id)
                        except errors.RPCError as e:
                            if getattr(e, 'message', None) != 'FILE_REQUEST_RECEIVED_ON_CONNECTION_SERVER':
                                raise
                            # We're already on a dedicated exported sender and
                            # the server still rejected it this way -- this is
