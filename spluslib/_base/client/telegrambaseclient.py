...
class SoroushPlusBaseClient(abc.ABC):
    ...
    Arguments
        session (`str` | `spluspy.sessions.abstract.Session`, `None`):
        ...
        connection (`spluspy.network.connection.common.Connection`, optional):
            The connection instance to be used when creating a new connection
            to the servers. It **must** be a type.

            Defaults to `spluspy.network.connection.tcpfull.ConnectionTcpFull`.
        ...
        app_version (`str`, optional):
            ...
            Defaults to `spluspy.version.__version__`.
    ...
    def __init__(
        ...
        if not api_id or not api_hash:
            raise ValueError(
                ...
                "Refer to spluspy.rtfd.io for more information.")
    ...
    async def connect(self: 'SoroushClient') -> None:
        ...
        .. note::
            ...
            request, like `client.get_me() <spluspy.client.users.UserMethods.get_me>`,
    ...
    async def _get_dc(self: 'SoroushClient', dc_id, cdn=False):
        ...
        SoroushPlus serves each DC on its own numbered subdomain,
        e.g. im-server-1.splus.ir, im-server-3.splus.ir, etc. (confirmed
        by capturing the real web client's WebSocket connections). The
        *default* host im-server.splus.ir (no number, see
        DEFAULT_IPV4_IP above) happens to be DC 3 specifically -- that's
        only correct for the account's own/main DC, and was wrongly
        being used here for every dc_id, including when borrowing an
        exported sender for uploads/downloads on a *different* DC. That
        mismatch is exactly what produced
        FILE_REQUEST_RECEIVED_ON_CONNECTION_SERVER: the borrowed sender
        was silently connecting to DC 3's server again instead of the
        DC it claimed to be borrowing for.
        """
        # Soroush: map each DC to its own numbered splus.ir server
        if not cdn:
            domain = '.splus.ir'
            if dc_id in (1, 2, 3, 4, 5, 6, 8):
                ip = f'im-server-{dc_id}{domain}'
            else:
                ip = None
    ...
    async def _create_exported_sender(self: 'SoroushClient', dc_id):
        ...
        Mirrors the real web client's `_connectSender`/`_createExportedSender`:
        when `dc_id` is the account's *own* DC, the sender is constructed
        with the main session's existing `auth_key` (`session.getAuthKey(dcId)`
        in the JS client) so `MTProtoSender.connect()` skips the DH handshake
        entirely and never calls `auth.ExportAuthorization` /
        `auth.ImportAuthorization`. Only for a genuinely *different* DC does
        the JS client's `if (this.session.dcId !== dcId && !sender._authenticated)`
        branch run and export/import a fresh key.
        This distinction matters here because SoroushPlus's per-DC file
        server (e.g. im-server-3.splus.ir) only understands upload/download
        RPCs and answers NORMAL_REQUEST_RECEIVED_ON_FILE_SERVER to anything
        else -- including the InitConnection/ImportAuthorization handshake
        the old code unconditionally sent.
        """
    ...
    def __call__(self: 'SoroushClient', request, ordered=False):
        ...
        Args:
            ...
            flood_sleep_threshold (`int` | `None`, optional):
                ...
                `client.flood_sleep_threshold <spluspy.client.telegrambaseclient.SoroushPlusBaseClient.flood_sleep_threshold>`
