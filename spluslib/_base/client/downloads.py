...
class _DirectDownloadIter(RequestIter):
    async def _init(
        ...
        # SoroushPlus's file server never accepts upload.GetFile on the
        # main/control connection (confirmed: it replies
        # FILE_REQUEST_RECEIVED_ON_CONNECTION_SERVER), so downloads must
        # always go through a borrowed exported sender too -- same fix as
        # upload_file(). _borrow_exported_sender already knows to reuse the
        # session's own auth_key (no export/import) when dc_id is the
        # account's own DC, so this is safe and cheap even in the common case.
        self._exported = True
        try:
            self._sender = await self.client._borrow_exported_sender(dc_id or self._client.session.dc_id)
...
class DownloadMethods:
    ...
    async def download_media(
        ...
        See also `Message.download_media() <spluspy.tl.custom.message.Message.download_media>`.

        Arguments
            message (`Message <spluspy.tl.custom.message.Message>` | :tl:`Media`):
    ...
    async def download_file(
        ...
        Arguments
            input_location (:tl:`InputFileLocation`):
                ...
                See `spluspy.utils.get_input_location` source for a complete
