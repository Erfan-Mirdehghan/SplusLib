...
class MTProtoSender:
    ...
    def __init__(self, auth_key, *, loggers,
        ...
        self._ping = None
        # FIX (spluspy): timestamp of the outstanding ping and a hard
        # timeout for it, used by _keepalive_ping() to force a reconnect
        # if a pong never arrives, instead of only detecting this on the
        # *next* scheduled keepalive call (see _keepalive_ping docstring).
        self._ping_sent_at = None
        self._PING_TIMEOUT = 20
        # FIX (spluspy): a single SecurityError from decrypt_message_data
        # is normal noise on a lossy network and is safe to ignore. But a
        # long unbroken run of them (as seen in the reported logs, right
        # before the connection was in fact dead) is a sign the session
        # state itself is corrupted rather than one bad packet, so past a
        # threshold we stop silently swallowing them and reconnect.
        self._consecutive_security_errors = 0
        self._MAX_CONSECUTIVE_SECURITY_ERRORS = 5
    ...
    async def _reconnect(self, last_error):
        ...
        for attempt in retry_range(retries, force_retry=False):
            try:
                ...
                # FIX (spluspy): stale PingRequests left over in
                # _pending_state from before the reconnect are pointless to
                # resend - nobody is awaiting their result (see
                # _keepalive_ping/_handle_pong, which never keep a future
                # around) and the connection just came back up, so a fresh
                # keepalive ping will be sent again shortly anyway. Without
                # this filter, a connection that flapped several times
                # before finally reconnecting could resend a growing pile
                # of dead pings on every attempt - the same symptom as the
                # reported log spam, just relocated to after reconnect
                # instead of before it.
                self._send_queue.extend(
                    state for state in self._pending_state.values()
                    if not isinstance(state.request, PingRequest)
                )
                self._pending_state.clear()
                self._ping = None
                self._ping_sent_at = None
    ...
    def _keepalive_ping(self, rnd_id):
        ...
        FIX (spluspy): the original version only ever looked at whether
        ``self._ping`` was still set, which relies on this method being
        called again *after* a full keepalive interval has already passed
        with no pong. Combined with the websocket layer silently hanging
        forever on a half-open socket (see websocket.py), this meant a
        dead connection could sit producing queued PingRequest after
        PingRequest with no reconnect ever triggering - which is exactly
        the symptom in the reported logs.

        Now we also track *when* the outstanding ping was sent and force
        a reconnect if it has been outstanding for longer than
        ``_PING_TIMEOUT``, instead of only reacting on the next scheduled
        call. This is a second, independent safety net on top of the
        websocket read-idle timeout - either one alone is enough to
        recover, but having both means a fix/regression in one layer
        doesn't leave the client silently stuck again.
        """
        now = helpers.get_running_loop().time()
        ...
        outstanding_for = now - (self._ping_sent_at or now)
        if outstanding_for >= self._PING_TIMEOUT:
            self._log.warning(
                'No pong received for %.1fs (ping %s); forcing reconnect',
                outstanding_for, self._ping)
            self._ping = None
            self._ping_sent_at = None
    ...
    async def _recv_loop(self):
        ...
        while self._user_connected and not self._reconnecting:
            ...
            try:
                ...
                self._consecutive_security_errors += 1
                if (self._consecutive_security_errors >=
                        self._MAX_CONSECUTIVE_SECURITY_ERRORS):
                    self._log.warning(
                        '%d consecutive security errors; session state '
                        'looks corrupted, forcing reconnect',
                        self._consecutive_security_errors)
                    self._consecutive_security_errors = 0
                    self._start_reconnect(e)
                    return
                continue
