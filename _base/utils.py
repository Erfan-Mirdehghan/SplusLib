...
def resolve_bot_file_id(file_id):
    """
    Given a Bot API-style `file_id <spluspy.tl.custom.file.File.id>`,
    returns the media it represents. If the `file_id <spluspy.tl.custom.file.File.id>`
    is not valid, `None` is returned instead.

    Note that the `file_id <spluspy.tl.custom.file.File.id>` does not have information
...
def split_text(text, entities, *, limit=4096, max_entities=100, split_at=(r'\n', r'\s', '.')):
    ...
    Example
        .. code-block:: python
            ...
            from spluspy.extensions import markdown
