...
class WebSocketReader:
    ...
    FIX (spluspy): the original implementation had no bound on how long a
    single ``await self._ws.receive()`` call could take. On a half-open
    TCP connection (very common on mobile networks: the phone changes
    network, sleeps, or a NAT/firewall silently drops the mapping) neither
    a TCP RST nor a WebSocket CLOSE frame ever arrives, so aiohttp's
    ``receive()`` just hangs forever with no exception and no CLOSE
    message. The library then never raises IOError here, never reaches
    ``MTProtoSender._recv_loop``'s except-block, and the socket is
    considered "alive" indefinitely - while ping requests keep silently
    piling up in ``_pending_state`` (exactly the log spam that motivated
    this rewrite). A per-read idle timeout closes that hole: if no frame
    is seen for ``_READ_IDLE_TIMEOUT`` seconds we raise IOError ourselves,
    which lets the normal reconnect machinery in MTProtoSender take over.
    """
    __slots__ = ('_ws', '_chunks', '_chunk_offset', '_total_len')

    # Slightly under half of _RECV_TIMEOUT (90s) in mtprotosender.py, so
    # a dead half-open socket is caught here first instead of relying on
    # that outer timeout as the only safety net.
    _READ_IDLE_TIMEOUT = 40
        ...
        while self._total_len < n:
            try:
                msg = await asyncio.wait_for(
                    self._ws.receive(), timeout=self._READ_IDLE_TIMEOUT)
            except asyncio.TimeoutError:
                _log.warning(
                    'WebSocket idle for %ds with no frame; treating '
                    'connection as dead', self._READ_IDLE_TIMEOUT)
                raise IOError(
                    'WebSocket idle timeout ({}s) while reading'.format(
            ...
            if msg.type == aiohttp.WSMsgType.ERROR:
                _log.debug('WebSocket ERROR frame: %s', self._ws.exception())
                break
            # aiohttp already answers PING frames and swallows PONG frames
            # internally when heartbeat is enabled, but be defensive in
            # case a raw PING/PONG ever reaches this layer - it carries no
            # payload for us and must not be treated as protocol data.
...
class WebSocketWriter:
    ...
    # FIX (spluspy): ws.send_bytes() has no internal timeout. On a
    # half-open socket the OS TCP send buffer can fill up while waiting
    # for ACKs from a peer that is no longer there, and this await would
    # then hang indefinitely - a write-side mirror of the read-side hang
    # fixed in WebSocketReader.readexactly. Bounding it here means a dead
    # connection is detected on whichever side (read or write) notices
    # first, instead of only ever relying on incoming traffic to time out.
    _WRITE_TIMEOUT = 20
        ...
        if self._pending:
            ...
            try:
                await asyncio.wait_for(
                    self._ws.send_bytes(data), timeout=self._WRITE_TIMEOUT)
            except asyncio.TimeoutError:
                raise IOError(
                    'WebSocket write timed out ({}s); connection is '
                    'likely dead'.format(self._WRITE_TIMEOUT))
